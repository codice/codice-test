...
import org.codice.pax.exam.junit.rules.ServiceAdmin;
 ...
 * installing, uninstalling, starting, or stopping a repository, a feature, or a bundle for example.
 *
...
public class TaskList {
   ...
   */
  private final Map<Operation, Map<String, Predicate<SnapshotReport>>> groups =
   ...
   * executing a particular group/operation of tasks for a given type of entities (i.e.
   * repositories, features or bundles).
   */
   ...
   */
  public TaskList(String type, SnapshotReport report) {
    this.type = type;
    this.types = type.endsWith("y") ? type.substring(0, type.length() - 1) + "ies" : type + 's';
    this.report = report;
    for (final Operation op : Operation.values()) {
      attemptsLeft.put(op, new AtomicInteger(ServiceAdmin.ATTEMPT_COUNT));
    }
  }
   ...
   */
  public void add(Operation op, String id, Predicate<SnapshotReport> task) {
  ...
  public <T> CompoundTask<T> addIfAbsent(
      Operation op, Supplier<T> containerFactory, BiPredicate<T, SnapshotReport> task) {
  ...
  public boolean execute() {
    LOGGER.debug("Executing {} tasks", types);
    ...
    if (LOGGER.isDebugEnabled()) {
      groups
          ...
          .forEach(
              e ->
                  LOGGER.debug(
                      "{} tasks recorded for {}: {}", e.getKey(), types, e.getValue().keySet()));
      compoundGroups
          .entrySet()
          .forEach(
              e ->
                  LOGGER.debug(
                      "{} compound tasks recorded for {}: {}", e.getKey(), types, e.getValue()));
    }
    ...
    if (n <= 0) { // too many attempts for this operation already, fail!
      LOGGER.debug("No more {} tasks attempts left for {}", opName, types);
      report.recordOnFinalAttempt(
          new ServiceException("Task error: too many " + type + "s " + opName + " attempts"));
      return false;
    }
    LOGGER.debug("{} tasks attempts left for {}: {}", opName, types, n);
    final Map<String, Predicate<SnapshotReport>> tasks = groups.get(op);

    try {
      boolean result = true; // until proven otherwise

      if (tasks != null) {
        Stream<Map.Entry<String, Predicate<SnapshotReport>>> s = tasks.entrySet().stream();
      ...
      if (compoundTask != null) {
        LOGGER.debug("Executing {} compound task for {}", opName, types);
  ...
  @VisibleForTesting
  Map<Operation, Map<String, Predicate<SnapshotReport>>> getTasks() {
    return groups;
  }
  ...
  public class CompoundTask<T> {
    ...
    private final BiPredicate<T, SnapshotReport> task;
    private final T container;
    private volatile int size;

    private CompoundTask(Operation op, BiPredicate<T, SnapshotReport> task, T container) {
      this(op, task, container, 0);
    }

    @VisibleForTesting
    CompoundTask(Operation op, BiPredicate<T, SnapshotReport> task, T container, int size) {
      this.operation = op;
      this.task = task;
    ...
    @Override
    public int hashCode() {
      return Objects.hash(size, operation, task, container);
    }
    ...
    private boolean test(SnapshotReport report) {
